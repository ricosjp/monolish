...
int main(){
  ...
  monolish::matrix::CRS<double> A(COO); // Create CRS format and convert from COO format
  ...
  monolish::standard_eigen::LOBPCG<monolish::matrix::CRS<double>, double> solver; 
  ...
  //Solve 
  monolish::util::solver_check(solver.solve(A, eigs, eigvecs)); 
  ...
  for(size_t i=0; i < eigs.size(); i++){
    ...
    // view1D is a 1D reference class that have same functions as monolish::vector
    monolish::view1D<monolish::matrix::Dense<double>, double> x(eigvecs, i*A.get_col(), (i+1)*A.get_col());
    x.print_all();
  }
