...
// double
int internal::lapack::sytrs(const matrix::Dense<double> &A, vector<double> &B, const std::vector<int>& ipiv){
...
#pragma omp target data use_device_ptr(Ad, ipivd, Bd, workd) 
    {
      internal::check_CUDA(cusolverDnDsytrs
          (h, CUBLAS_FILL_MODE_UPPER, M, nrhs, Ad, N, ipivd, Bd, M, workd, lwork, &info));
    }
