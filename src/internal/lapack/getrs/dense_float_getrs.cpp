...
// float
int internal::lapack::getrs(const matrix::Dense<float> &A, vector<float> &B, const std::vector<int>& ipiv){
...
#pragma omp target data use_device_ptr(Ad, ipivd, Bd) 
    {
      internal::check_CUDA(cusolverDnSgetrs(h, CUBLAS_OP_N, M, K, Ad, N, ipivd, Bd, M, &info));
    }
