...
void times_core(const T alpha, const MAT1 &A, MAT2 &C) {
...
void times_row_core(const MAT1 &A, const VEC &x, MAT2 &C) {
  Logger &logger = Logger::get_instance();
  logger.func_in(monolish_func);
...
template <typename VEC, typename MAT1, typename MAT2>
void times_col_core(const MAT1 &A, const VEC &x, MAT2 &C) {
  Logger &logger = Logger::get_instance();
  logger.func_in(monolish_func);
