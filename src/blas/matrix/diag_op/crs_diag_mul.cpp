...
// mul scalar
template <typename T> void CRS<T>::diag_mul(const T alpha) {
...
// mul vector
template <typename T> void CRS<T>::diag_mul(const vector<T> &vec) {
  Logger &logger = Logger::get_instance();
  logger.func_in(monolish_func);
...
// mul viwe1D<Dense>
template <typename T>
void CRS<T>::diag_mul(const view1D<matrix::Dense<T>, T> &vec) {
  Logger &logger = Logger::get_instance();
  logger.func_in(monolish_func);
