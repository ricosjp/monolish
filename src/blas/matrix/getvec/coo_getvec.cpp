...
// diag
template <typename T> vector<T> COO<T>::diag() const {
...
template vector<double> monolish::matrix::COO<double>::diag() const;
template vector<float> monolish::matrix::COO<float>::diag() const;
...
// row
template <typename T> vector<T> COO<T>::row(const size_t r) const {
  Logger &logger = Logger::get_instance();
  logger.func_in(monolish_func);

  vector<T> res(get_col(), 0);
  for (std::size_t nz = 0; nz < get_nnz(); ++nz) {
...
// col
template <typename T> vector<T> COO<T>::col(const size_t c) const {
  Logger &logger = Logger::get_instance();
  logger.func_in(monolish_func);

  vector<T> res(get_row(), 0);
  for (std::size_t nz = 0; nz < get_nnz(); ++nz) {
