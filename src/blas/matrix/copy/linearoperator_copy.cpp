...
template <typename T> void LinearOperator<T>::operator=(const LinearOperator<T> &linearoperator){
  Logger &logger = Logger::get_instance();
  logger.util_in(monolish_func);
...
template void LinearOperator<double>::operator=(const LinearOperator<double> &linearoperator);
template void LinearOperator<float>::operator=(const LinearOperator<float> &linearoperator);
