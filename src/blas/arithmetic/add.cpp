...
/////////////////////////////////////////////////
template <typename T>
  void blas::add(const vector<T>& a, const T alpha, vector<T>& y){
  Logger &logger = Logger::get_instance();
  logger.func_in(monolish_func);
...
template <typename T> vector<T> vector<T>::operator+(const T value) {
  ...
  internal::vadd(val.size(), val.data(), value, ans.data(), gpu_status);
...
template <typename T> void vector<T>::operator+=(const T value) {
  ...
  internal::vadd(val.size(), val.data(), value, val.data(), gpu_status);
...
template <typename T> vector<T> vector<T>::operator+(const vector<T> &vec) {
  ...
  internal::vadd(val.size(), val.data(), vec.data(), ans.data(), gpu_status);
...
template <typename T> void vector<T>::operator+=(const vector<T> &vec) {
  ...
  internal::vadd(val.size(), val.data(), vec.data(), val.data(), gpu_status);
