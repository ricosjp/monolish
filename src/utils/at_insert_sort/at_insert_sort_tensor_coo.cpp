...
template <typename T> T tensor_COO<T>::at(const std::vector<size_t>& pos) const {
...
template <typename T> void tensor_COO<T>::_q_sort(int lo, int hi){
  ...
  auto p = hi;
  auto p1 = index[p];
  auto indl = get_index(index[l]);
  auto indp = get_index(index[p]);
  auto indh = get_index(index[h]);
  double p3 = data()[p];

  do {
    while ((l < h) && ((indl - indp)) <= 0) {
      l = l + 1;
      indl = get_index(index[l]);
    }
    while((h > l) && ((indh - indp)) >= 0) {
      h = h - 1;
      indh = get_index(index[h]);
    }
    if(l < h){
      auto t = index[l];
      index[l] = index[h];
...
template <typename T>
void tensor_COO<T>::sort(bool merge) {
