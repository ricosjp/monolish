...
template <typename T>
void ans_vvpow(monolish::vector<T> &mx, monolish::vector<T> &my,
  ...
  for (size_t i = 0; i < mx.size(); i++) {
    ans[i] = std::pow(mx[i], my[i]);
  }
}

template <typename T> bool test_send_vvpow(const size_t size, double tol) {
  ...
  ans_vvpow(x, y, ans_tmp);

  monolish::util::send(x, y, ans);
  monolish::vml::pow(x, y, ans);
  ans.recv();

  return ans_check<T>(__func__, ans.data(), ans_tmp.data(), x.size(), tol);
}

template <typename T> bool test_vvpow(const size_t size, double tol) {
