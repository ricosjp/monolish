...
template <typename T>
void ans_vvmul(monolish::vector<T> &mx, monolish::vector<T> &my,
...
template <typename T> bool test_send_vvmul(const size_t size, double tol) {
  ...
  monolish::vml::mul(x, y, ans);
  ans.recv();

  return ans_check<T>(__func__, ans.begin(), ans_tmp.begin(), x.size(), tol);
}

template <typename T> bool test_vvmul(const size_t size, double tol) {
