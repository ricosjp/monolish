...
template <typename T> bool test_send_vecadd(const size_t size, double tol) {
  ...
  ans_vecadd(x, y, ans_tmp);

  monolish::util::send(x, y, ans);
  monolish::blas::vecadd(x, y, ans);
  ans.recv();

  return ans_check<T>(__func__, ans.data(), ans_tmp.data(), x.size(), tol);
}

template <typename T> bool test_vecadd(const size_t size, double tol) {
