...
template <typename T>
void get_ans(monolish::vector<T> &mx, monolish::vector<T> &my, monolish::vector<T> &ans){
	if(mx.size() != my.size()){
...
bool test(const size_t size, double tol, const size_t iter, const size_t check_ans){
    ...
    std::cout << "func\tprec\tsize\ttime[sec]\tperf[GFLOPS]\tmem[GB/s] " << std::endl;
...
int main(int argc, char** argv){
    ...
    if(argc!=5){
        std::cout << "error!, $1:precision (double or float), $2:vector size, $3: iter, $4: error check (1/0)" << std::endl;
        return 1;
    }
