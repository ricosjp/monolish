...
void ans_adds_col(const monolish::tensor::tensor_Dense<T> &A, const VEC &mx,
...
bool test_send_adds_col_core(const size_t M, const size_t N, const size_t L,
...
bool test_send_adds_col(const size_t M, const size_t N, const size_t L,
  ...
  return test_send_adds_col_core(M, N, L, tol, AA, x, CC, A, x, C);
}

template <typename T, typename TENS_A, typename VEC, std::size_t K = 0,
          typename... Tp>
inline typename std::enable_if<K == sizeof...(Tp), bool>::type
test_send_adds_col_view_core3(const size_t M, const size_t N, const size_t L,
                              double tol, monolish::tensor::tensor_Dense<T> &AA,
                              monolish::vector<T> &vec,
                              monolish::tensor::tensor_Dense<T> &CC, TENS_A &A,
...
template <typename T, typename TENS_A, typename VEC, std::size_t K = 0,
    ...
    K<sizeof...(Tp), bool>::type test_send_adds_col_view_core3(
  ...
  A = AA;
  x = vec;
  if (!test_send_adds_col_core(M, N, L, tol, AA, vec, CC, A, x,
                               std::get<K>(Cs))) {
    return false;
  }
  return test_send_adds_col_view_core3<T, TENS_A, VEC, K + 1, Tp...>(
...
test_send_adds_col_view_core2(const size_t M, const size_t N, const size_t L,
                              double tol, monolish::tensor::tensor_Dense<T> &AA,
                              monolish::vector<T> &vec,
                              monolish::tensor::tensor_Dense<T> &CC, TENS_A &A,
...
template <typename T, typename TENS_A, std::size_t J = 0, typename TENSES_C,
          typename... Tq>
    inline typename std::enable_if <
    J<sizeof...(Tq), bool>::type test_send_adds_col_view_core2(
        const size_t M, const size_t N, const size_t L, double tol,
        monolish::tensor::tensor_Dense<T> &AA, monolish::vector<T> &vec,
        monolish::tensor::tensor_Dense<T> &CC, TENS_A &A, std::tuple<Tq...> &xs,
        TENSES_C &Cs) {
  if (!test_send_adds_col_view_core3(M, N, L, tol, AA, vec, CC, A,
                                     std::get<J>(xs), Cs)) {
    return false;
  }
  return test_send_adds_col_view_core2<T, TENS_A, J + 1, TENSES_C, Tq...>(
      M, N, L, tol, AA, vec, CC, A, xs, Cs);
}

template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_adds_col_view_core1(const size_t M, const size_t N, const size_t L,
                              double tol, monolish::tensor::tensor_Dense<T> &AA,
                              monolish::vector<T> &vec,
...
template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
    ...
    I<sizeof...(Tp), bool>::type test_send_adds_col_view_core1(
        ...
        std::tuple<Tp...> &Cs) {
  if (!test_send_adds_col_view_core2(M, N, L, tol, AA, vec, CC, std::get<I>(As),
                                     xs, Cs)) {
    return false;
  }
...
template <typename T>
bool test_send_adds_col_view(const size_t M, const size_t N, const size_t L,
                             double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  ...
  return test_send_adds_col_view_core1(M, N, L, tol, AA, xx, CC, As, Bs, Cs);
}

template <typename T, typename TENS_A, typename TENS_C, typename VEC>
bool test_adds_col_core(const size_t M, const size_t N, const size_t L,
                        double tol, monolish::tensor::tensor_Dense<T> &AA,
                        monolish::vector<T> &vec,
                        monolish::tensor::tensor_Dense<T> &CC, TENS_A &A,
                        VEC &x, TENS_C &C) {
...
bool test_adds_col(const size_t M, const size_t N, const size_t L, double tol) {
  ...
  return test_adds_col_core(M, N, L, tol, AA, x, CC, A, x, C);
}

template <typename T, typename TENS_A, typename VEC, std::size_t K = 0,
          typename... Tp>
inline typename std::enable_if<K == sizeof...(Tp), bool>::type
test_adds_col_view_core3(const size_t M, const size_t N, const size_t L,
                         double tol, monolish::tensor::tensor_Dense<T> &AA,
                         monolish::vector<T> &vec,
                         monolish::tensor::tensor_Dense<T> &CC, TENS_A &A,
                         VEC &x, std::tuple<Tp...> &Cs) {
...
template <typename T, typename TENS_A, typename VEC, std::size_t K = 0,
  ...
  A = AA;
  x = vec;
  if (!test_adds_col_core(M, N, L, tol, AA, vec, CC, A, x, std::get<K>(Cs))) {
    return false;
  }
  return test_adds_col_view_core3<T, TENS_A, VEC, K + 1, Tp...>(
      M, N, L, tol, AA, vec, CC, A, x, Cs);
}
...
template <typename T, typename TENS_A, std::size_t J = 0, typename TENSES_C,
          typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_adds_col_view_core2(const size_t M, const size_t N, const size_t L,
                         double tol, monolish::tensor::tensor_Dense<T> &AA,
                         monolish::vector<T> &vec,
                         monolish::tensor::tensor_Dense<T> &CC, TENS_A &A,
                         std::tuple<Tq...> &x, TENSES_C &Cs) {
...
template <typename T, typename TENS_A, std::size_t J = 0, typename TENSES_C,
  ...
  if (!test_adds_col_view_core3(M, N, L, tol, AA, vec, CC, A, std::get<J>(xs),
                                Cs)) {
    return false;
  }
  return test_adds_col_view_core2<T, TENS_A, J + 1, TENSES_C, Tq...>(
      M, N, L, tol, AA, vec, CC, A, xs, Cs);
}

template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_adds_col_view_core1(const size_t M, const size_t N, const size_t L,
                         double tol, monolish::tensor::tensor_Dense<T> &AA,
                         monolish::vector<T> &vec,
                         monolish::tensor::tensor_Dense<T> &CC,
                         std::tuple<Tp...> &As, VECS &xs,
...
template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
    ...
    I<sizeof...(Tp), bool>::type test_adds_col_view_core1(
        ...
        monolish::tensor::tensor_Dense<T> &CC, std::tuple<Tp...> &As, VECS &xs,
        std::tuple<Tp...> &Cs) {
  if (!test_adds_col_view_core2(M, N, L, tol, AA, vec, CC, std::get<I>(As), xs,
                                Cs)) {
    return false;
  }
  return test_adds_col_view_core1<T, I + 1, VECS, Tp...>(M, N, L, tol, AA, vec,
                                                         CC, As, xs, Cs);
}

template <typename T>
bool test_adds_col_view(const size_t M, const size_t N, const size_t L,
                        double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  monolish::tensor::tensor_Dense<T> AA(seedA), CC(seedA);
  ...
  return test_adds_col_view_core1(M, N, L, tol, AA, xx, CC, As, Bs, Cs);
}
