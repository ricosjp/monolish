...
bool test_send_tenssub_core(const size_t M, const size_t N, const size_t L,
...
bool test_send_tenssub(const size_t M, const size_t N, const size_t L,
                       double tol) {
  ...
  return test_send_tenssub_core(M, N, L, tol, AA, BB, CC, A, B, C);
}

template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
          typename... Tp>
inline typename std::enable_if<K == sizeof...(Tp), bool>::type
test_send_tenssub_view_core3(const size_t M, const size_t N, const size_t L,
                             double tol, monolish::tensor::tensor_Dense<T> &AA,
                             monolish::tensor::tensor_Dense<T> &BB,
                             monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
...
template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
  ...
  A = AA;
  B = BB;
  if (!test_send_tenssub_core(M, N, L, tol, AA, BB, CC, A, B,
                              std::get<K>(Cs))) {
    return false;
  }
  return test_send_tenssub_view_core3<T, MAT_A, MAT_B, K + 1, Tp...>(
...
test_send_tenssub_view_core2(const size_t M, const size_t N, const size_t L,
                             double tol, monolish::tensor::tensor_Dense<T> &AA,
                             monolish::tensor::tensor_Dense<T> &BB,
                             monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
    inline typename std::enable_if <
    J<sizeof...(Tp), bool>::type test_send_tenssub_view_core2(
        const size_t M, const size_t N, const size_t L, double tol,
        monolish::tensor::tensor_Dense<T> &AA,
        monolish::tensor::tensor_Dense<T> &BB,
        monolish::tensor::tensor_Dense<T> &CC, MAT_A &A, std::tuple<Tp...> &Bs,
        std::tuple<Tp...> &Cs) {
  if (!test_send_tenssub_view_core3(M, N, L, tol, AA, BB, CC, A,
                                    std::get<J>(Bs), Cs)) {
    return false;
  }
  return test_send_tenssub_view_core2<T, MAT_A, J + 1, Tp...>(
      M, N, L, tol, AA, BB, CC, A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_tenssub_view_core1(const size_t M, const size_t N, const size_t L,
                             double tol, monolish::tensor::tensor_Dense<T> &AA,
                             monolish::tensor::tensor_Dense<T> &BB,
                             monolish::tensor::tensor_Dense<T> &CC,
...
template <typename T, std::size_t I = 0, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type test_send_tenssub_view_core1(
        const size_t M, const size_t N, const size_t L, double tol,
        monolish::tensor::tensor_Dense<T> &AA,
        monolish::tensor::tensor_Dense<T> &BB,
        monolish::tensor::tensor_Dense<T> &CC, std::tuple<Tp...> &As,
        std::tuple<Tp...> &Bs, std::tuple<Tp...> &Cs) {
  if (!test_send_tenssub_view_core2(M, N, L, tol, AA, BB, CC, std::get<I>(As),
                                    Bs, Cs)) {
    return false;
  }
  return test_send_tenssub_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, BB, CC,
                                                       As, Bs, Cs);
}

template <typename T>
bool test_send_tenssub_view(const size_t M, const size_t N, const size_t L,
                            double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  ...
  return test_send_tenssub_view_core1(M, N, L, tol, AA, BB, CC, As, Bs, Cs);
}

template <typename T, typename MAT_A, typename MAT_B, typename MAT_C>
bool test_tenssub_core(const size_t M, const size_t N, const size_t L,
                       double tol, monolish::tensor::tensor_Dense<T> &AA,
                       monolish::tensor::tensor_Dense<T> &BB,
                       monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                       MAT_B &B, MAT_C &C) {
...
bool test_tenssub(const size_t M, const size_t N, const size_t L, double tol) {
  ...
  return test_tenssub_core(M, N, L, tol, AA, BB, CC, A, B, C);
}

template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
          typename... Tp>
inline typename std::enable_if<K == sizeof...(Tp), bool>::type
test_tenssub_view_core3(const size_t M, const size_t N, const size_t L,
                        double tol, monolish::tensor::tensor_Dense<T> &AA,
                        monolish::tensor::tensor_Dense<T> &BB,
                        monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                        MAT_B &B, std::tuple<Tp...> &Cs) {
...
template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
  ...
  A = AA;
  B = BB;
  if (!test_tenssub_core(M, N, L, tol, AA, BB, CC, A, B, std::get<K>(Cs))) {
    return false;
  }
  return test_tenssub_view_core3<T, MAT_A, MAT_B, K + 1, Tp...>(
      M, N, L, tol, AA, BB, CC, A, B, Cs);
}

template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
inline typename std::enable_if<J == sizeof...(Tp), bool>::type
test_tenssub_view_core2(const size_t M, const size_t N, const size_t L,
                        double tol, monolish::tensor::tensor_Dense<T> &AA,
                        monolish::tensor::tensor_Dense<T> &BB,
                        monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                        std::tuple<Tp...> &Bs, std::tuple<Tp...> &Cs) {
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
    ...
    test_tenssub_view_core2(const size_t M, const size_t N, const size_t L,
                            ...
                            monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                            std::tuple<Tp...> &Bs, std::tuple<Tp...> &Cs) {
  if (!test_tenssub_view_core3(M, N, L, tol, AA, BB, CC, A, std::get<J>(Bs),
                               Cs)) {
    return false;
  }
  return test_tenssub_view_core2<T, MAT_A, J + 1, Tp...>(M, N, L, tol, AA, BB,
                                                         CC, A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_tenssub_view_core1(const size_t M, const size_t N, const size_t L,
                        double tol, monolish::tensor::tensor_Dense<T> &AA,
                        monolish::tensor::tensor_Dense<T> &BB,
                        monolish::tensor::tensor_Dense<T> &CC,
                        std::tuple<Tp...> &As, std::tuple<Tp...> &Bs,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_tenssub_view_core1(const size_t M, const size_t N, const size_t L,
                            ...
                            std::tuple<Tp...> &As, std::tuple<Tp...> &Bs,
                            std::tuple<Tp...> &Cs) {
  if (!test_tenssub_view_core2(M, N, L, tol, AA, BB, CC, std::get<I>(As), Bs,
                               Cs)) {
    return false;
  }
  return test_tenssub_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, BB, CC, As,
                                                  Bs, Cs);
}

template <typename T>
bool test_tenssub_view(const size_t M, const size_t N, const size_t L,
                       double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  ...
  return test_tenssub_view_core1(M, N, L, tol, AA, BB, CC, As, Bs, Cs);
}
