...
void ans_times_row(const monolish::tensor::tensor_Dense<T> &A,
                   const VEC &mx,
...
bool test_send_times_row_core(const size_t M, const size_t N, const size_t L, VEC& x,
                         double tol) {
  size_t nnzrow = 27;
  if (nnzrow < L) {
    nnzrow = 27;
...
bool test_send_times_row(const size_t M, const size_t N, const size_t L, double tol){
  monolish::vector<T> vec(L, 0.0, 1.0, test_random_engine());
  return test_send_times_row_core<MAT, monolish::vector<T>, T>(M, N, L, vec, tol);
}
...
template <typename MAT, typename T>
bool test_times_row(const size_t M, const size_t N, const size_t L, double tol){
  monolish::vector<T> vec(L, 0.0, 1.0, test_random_engine());
...
bool test_send_times_row_view(const size_t M, const size_t N, const size_t L, double tol){
  U x(L, 0.0, 1.0);
  monolish::view1D<U, T> vec(x, 0, L);
  return test_send_times_row_core<MAT, monolish::view1D<U, T>, T>(M, N, L, vec, tol);
}
...
template <typename MAT, typename T, typename U, typename std::enable_if<std::is_same<U, monolish::vector<T>>::value, std::nullptr_t>::type = nullptr>
bool test_times_row_view(const size_t M, const size_t N, const size_t L, double tol){
  U x(L, 0.0, 1.0);
  monolish::view1D<U, T> vec(x, 0, L);
