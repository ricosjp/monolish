...
bool test_send_tt_copy_core(const size_t M, const size_t N, const size_t L,
...
bool test_send_tt_copy(const size_t M, const size_t N, const size_t L,
  ...
  return test_send_tt_copy_core(M, N, L, tol, AA, CC, A, C);
}

template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
inline typename std::enable_if<J == sizeof...(Tp), bool>::type
test_send_tt_copy_view_core2(const size_t M, const size_t N, const size_t L,
                             double tol, monolish::tensor::tensor_Dense<T> &AA,
                             monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                             std::tuple<Tp...> &Cs) {
  return true;
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
  ...
  A = AA;
  if (!test_send_tt_copy_core(M, N, L, tol, AA, CC, A, std::get<J>(Cs))) {
    return false;
  }
  return test_send_tt_copy_view_core2<T, MAT_A, J + 1, Tp...>(M, N, L, tol, AA,
                                                              CC, A, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_tt_copy_view_core1(const size_t M, const size_t N, const size_t L,
                             double tol, monolish::tensor::tensor_Dense<T> &AA,
                             monolish::tensor::tensor_Dense<T> &CC,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_send_tt_copy_view_core1(const size_t M, const size_t N, const size_t L,
                                 ...
                                 monolish::tensor::tensor_Dense<T> &CC,
                                 std::tuple<Tp...> &As, std::tuple<Tp...> &Cs) {
  if (!test_send_tt_copy_view_core2(M, N, L, tol, AA, CC, std::get<I>(As),
                                    Cs)) {
    return false;
  }
  return test_send_tt_copy_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, CC, As,
                                                       Cs);
}

template <typename T>
bool test_send_tt_copy_view(const size_t M, const size_t N, const size_t L,
                            double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  ...
  return test_send_tt_copy_view_core1(M, N, L, tol, AA, CC, As, Cs);
}

template <typename T, typename MAT_A, typename MAT_C>
bool test_tt_copy_core(const size_t M, const size_t N, const size_t L,
                       double tol, monolish::tensor::tensor_Dense<T> &AA,
                       monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                       MAT_C &C) {
  ans_tt_copy(AA, CC);
...
bool test_tt_copy(const size_t M, const size_t N, const size_t L, double tol) {
  ...
  return test_tt_copy_core(M, N, L, tol, AA, CC, A, C);
}

template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
inline typename std::enable_if<J == sizeof...(Tp), bool>::type
test_tt_copy_view_core2(const size_t M, const size_t N, const size_t L,
                        double tol, monolish::tensor::tensor_Dense<T> &AA,
                        monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                        std::tuple<Tp...> &Cs) {
  return true;
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
    ...
    test_tt_copy_view_core2(const size_t M, const size_t N, const size_t L,
                            ...
                            std::tuple<Tp...> &Cs) {
  A = AA;
  if (!test_tt_copy_core(M, N, L, tol, AA, CC, A, std::get<J>(Cs))) {
    return false;
  }
  return test_tt_copy_view_core2<T, MAT_A, J + 1, Tp...>(M, N, L, tol, AA, CC,
                                                         A, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_tt_copy_view_core1(const size_t M, const size_t N, const size_t L,
                        double tol, monolish::tensor::tensor_Dense<T> &AA,
                        monolish::tensor::tensor_Dense<T> &CC,
                        std::tuple<Tp...> &As, std::tuple<Tp...> &Cs) {
  return true;
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_tt_copy_view_core1(const size_t M, const size_t N, const size_t L,
                            ...
                            monolish::tensor::tensor_Dense<T> &CC,
                            std::tuple<Tp...> &As, std::tuple<Tp...> &Cs) {
  if (!test_tt_copy_view_core2(M, N, L, tol, AA, CC, std::get<I>(As), Cs)) {
    return false;
  }
  return test_tt_copy_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, CC, As, Cs);
}

template <typename T>
bool test_tt_copy_view(const size_t M, const size_t N, const size_t L,
                       double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  ...
  return test_tt_copy_view_core1(M, N, L, tol, AA, CC, As, Cs);
}
