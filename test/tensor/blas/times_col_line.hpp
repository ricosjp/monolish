...
bool test_send_times_col_line(const size_t M, const size_t N, const size_t L,
  ...
  vec.send();
  return test_send_times_col_line_core<MAT, monolish::vector<T>, T>(M, N, L,
                                                                    vec, tol);
}
...
bool test_times_col_line(const size_t M, const size_t N, const size_t L,
                         double tol) {
  monolish::vector<T> vec(M, 0.0, 1.0, test_random_engine());
  return test_times_col_line_core<MAT, monolish::vector<T>, T>(M, N, L, vec,
                                                               tol);
}
...
M, const size_t N, const size_t L, double tol){ U x(M, 0.0, 1.0);
  ...
  return test_send_times_col_line_core<MAT, monolish::view1D<U, T>, T>(M, N, L,
vec, tol);
}
...
bool test_times_col_line_view(const size_t M, const size_t N, const size_t L,
  ...
  U x(M, 0.0, 1.0);
  monolish::view1D<U, T> vec(x, 0, M);
  return test_times_col_line_core<MAT, monolish::view1D<U, T>, T>(M, N, L, vec,
                                                                  tol);
}
