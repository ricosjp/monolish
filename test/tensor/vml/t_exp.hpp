...
bool test_send_texp_core(const size_t M, const size_t N, const size_t L,
                         double tol, monolish::tensor::tensor_Dense<T> &AA,
                         MAT1 &A, MAT2 &B) {
  ans_exp(AA);
  monolish::util::send(A, B);
  monolish::vml::exp(A, B);
...
bool test_send_texp(const size_t M, const size_t N, const size_t L,
  ...
  return test_send_texp_core<T, MAT, MAT>(M, N, L, tol, AA, A, A);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_send_texp_view_core2(const size_t M, const size_t N, const size_t L,
                          double tol, monolish::tensor::tensor_Dense<T> &AA,
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    ...
    test_send_texp_view_core2(const size_t M, const size_t N, const size_t L,
                              ...
                              MAT1 &A, std::tuple<Tq...> &Bs) {
  A = AA;
  if (!test_send_texp_core<T, MAT1, decltype(std::get<J>(Bs))>(
          M, N, L, tol, AA, A, std::get<J>(Bs))) {
    return false;
  }
  return test_send_texp_view_core2<T, MAT1, J + 1, Tq...>(M, N, L, tol, AA, A,
                                                          Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_texp_view_core1(const size_t M, const size_t N, const size_t L,
                          double tol, monolish::tensor::tensor_Dense<T> &AA,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_send_texp_view_core1(const size_t M, const size_t N, const size_t L,
                              double tol, monolish::tensor::tensor_Dense<T> &AA,
                              std::tuple<Tp...> &As, std::tuple<Tp...> &Bs) {
  if (!test_send_texp_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, L, tol, AA, std::get<I>(As), Bs)) {
    return false;
  }
  return test_send_texp_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, As, Bs);
}

template <typename T>
bool test_send_texp_view(const size_t M, const size_t N, const size_t L,
                         double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  monolish::tensor::tensor_Dense<T> AA(seedA);
  ...
  return test_send_texp_view_core1<T, 0, T1, T2, T3, T4>(M, N, L, tol, AA, As,
                                                         Bs);
}

template <typename T, typename MAT1, typename MAT2>
bool test_texp_core(const size_t M, const size_t N, const size_t L, double tol,
...
bool test_texp(const size_t M, const size_t N, const size_t L, double tol) {
  ...
  return test_texp_core<T, MAT, MAT>(M, N, L, tol, AA, A, A);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_texp_view_core2(const size_t M, const size_t N, const size_t L, double tol,
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    inline typename std::enable_if <
    J<sizeof...(Tq), bool>::type
    test_texp_view_core2(const size_t M, const size_t N, const size_t L,
                         double tol, monolish::tensor::tensor_Dense<T> &AA,
                         MAT1 &A, std::tuple<Tq...> &Bs) {
  A = AA;
  if (!test_texp_core<T, MAT1, decltype(std::get<J>(Bs))>(M, N, L, tol, AA, A,
                                                          std::get<J>(Bs))) {
    return false;
  }
  return test_texp_view_core2<T, MAT1, J + 1, Tq...>(M, N, L, tol, AA, A, Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_texp_view_core1(const size_t M, const size_t N, const size_t L, double tol,
...
template <typename T, std::size_t I = 0, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type
    test_texp_view_core1(const size_t M, const size_t N, const size_t L,
                         double tol, monolish::tensor::tensor_Dense<T> &AA,
                         std::tuple<Tp...> &As, std::tuple<Tp...> &Bs) {
  if (!test_texp_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, L, tol, AA, std::get<I>(As), Bs)) {
    return false;
  }
  return test_texp_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, As, Bs);
}

template <typename T>
bool test_texp_view(const size_t M, const size_t N, const size_t L,
                    double tol) {
  monolish::tensor::tensor_COO<T> seedA =
      get_random_structure_tensor<T>(M, N, L);
  monolish::tensor::tensor_Dense<T> AA(seedA);
  ...
  return test_texp_view_core1<T, 0, T1, T2, T3, T4>(M, N, L, tol, AA, As, Bs);
}
