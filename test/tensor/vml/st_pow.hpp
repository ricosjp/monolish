...
bool test_send_st_pow_core(const size_t M, const size_t N, const size_t L,
...
bool test_send_st_pow(const size_t M, const size_t N, const size_t L,
  ...
  return test_send_st_pow_core(M, N, L, tol, AA, CC, A, C);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_send_st_pow_view_core2(const size_t M, const size_t N, const size_t L,
                            double tol, monolish::tensor::tensor_Dense<T> &AA,
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    ...
    J<sizeof...(Tq), bool>::type test_send_st_pow_view_core2(
  ...
  A = AA;
  if (!test_send_st_pow_core<T, MAT1, decltype(std::get<J>(Bs))>(
          M, N, L, tol, AA, CC, A, std::get<J>(Bs))) {
    return false;
  }
  return test_send_st_pow_view_core2<T, MAT1, J + 1, Tq...>(M, N, L, tol, AA,
                                                            CC, A, Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_st_pow_view_core1(const size_t M, const size_t N, const size_t L,
                            double tol, monolish::tensor::tensor_Dense<T> &AA,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_send_st_pow_view_core1(const size_t M, const size_t N, const size_t L,
                                ...
                                monolish::tensor::tensor_Dense<T> &CC,
                                std::tuple<Tp...> &As, std::tuple<Tp...> &Bs) {
  if (!test_send_st_pow_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, L, tol, AA, CC, std::get<I>(As), Bs)) {
    return false;
  }
  return test_send_st_pow_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, CC, As,
                                                      Bs);
}

template <typename T>
bool test_send_st_pow_view(const size_t M, const size_t N, const size_t L,
                           double tol) {
  monolish::tensor::tensor_Dense<T> seed1({M, N, L}, 1.0, 2.0);
  monolish::tensor::tensor_COO<T> seedA(seed1);
  ...
  return test_send_st_pow_view_core1<T, 0, T1, T2, T3, T4>(M, N, L, tol, AA, CC,
                                                           As, Bs);
}

template <typename T, typename MAT_A, typename MAT_C>
bool test_st_pow_core(const size_t M, const size_t N, const size_t L,
                      double tol, monolish::tensor::tensor_Dense<T> &AA,
                      monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                      MAT_C &C) {
  T alpha = 123.0;
...
bool test_st_pow(const size_t M, const size_t N, const size_t L, double tol) {
  ...
  return test_st_pow_core(M, N, L, tol, AA, CC, A, C);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_st_pow_view_core2(const size_t M, const size_t N, const size_t L,
                       double tol, monolish::tensor::tensor_Dense<T> &AA,
                       monolish::tensor::tensor_Dense<T> &CC, MAT1 &A,
                       std::tuple<Tq...> &Bs) {
  return true;
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    ...
    test_st_pow_view_core2(const size_t M, const size_t N, const size_t L,
                           ...
                           std::tuple<Tq...> &Bs) {
  A = AA;
  if (!test_st_pow_core<T, MAT1, decltype(std::get<J>(Bs))>(
          M, N, L, tol, AA, CC, A, std::get<J>(Bs))) {
    return false;
  }
  return test_st_pow_view_core2<T, MAT1, J + 1, Tq...>(M, N, L, tol, AA, CC, A,
                                                       Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_st_pow_view_core1(const size_t M, const size_t N, const size_t L,
                       double tol, monolish::tensor::tensor_Dense<T> &AA,
                       monolish::tensor::tensor_Dense<T> &CC,
                       std::tuple<Tp...> &As, std::tuple<Tp...> &Bs) {
  return true;
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_st_pow_view_core1(const size_t M, const size_t N, const size_t L,
                           ...
                           monolish::tensor::tensor_Dense<T> &CC,
                           std::tuple<Tp...> &As, std::tuple<Tp...> &Bs) {
  if (!test_st_pow_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, L, tol, AA, CC, std::get<I>(As), Bs)) {
    return false;
  }
  return test_st_pow_view_core1<T, I + 1, Tp...>(M, N, L, tol, AA, CC, As, Bs);
}

template <typename T>
bool test_st_pow_view(const size_t M, const size_t N, const size_t L,
                      double tol) {
  monolish::tensor::tensor_Dense<T> seed1({M, N, L}, 1.0, 2.0);
  monolish::tensor::tensor_COO<T> seedA(seed1);
  monolish::tensor::tensor_Dense<T> AA(seedA), CC(seedA);
  ...
  return test_st_pow_view_core1<T, 0, T1, T2, T3, T4>(M, N, L, tol, AA, CC, As,
                                                      Bs);
}
