...
bool test_send_tt_max_core(const size_t M, const size_t N, const size_t L,
...
bool test_send_tt_max(const size_t M, const size_t N, const size_t L,
  ...
  return test_send_tt_max_core<T, MAT_A, MAT_B, MAT_C>(M, N, L, tol, AA, BB, CC,
                                                       A, B, C);
}
...
test_send_tt_max_view_core3(const size_t M, const size_t N, const size_t L,
                            double tol, monolish::tensor::tensor_Dense<T> &AA,
                            monolish::tensor::tensor_Dense<T> &BB,
                            monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
...
template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
  ...
  A = AA;
  B = BB;
  if (!test_send_tt_max_core<T, MAT_A, MAT_B, decltype(std::get<K>(Cs))>(
          M, N, L, tol, AA, BB, CC, A, B, std::get<K>(Cs))) {
    return false;
  }
  return test_send_tt_max_view_core3<T, MAT_A, MAT_B, K + 1, Tr...>(
...
test_send_tt_max_view_core2(const size_t M, const size_t N, const size_t L,
                            double tol, monolish::tensor::tensor_Dense<T> &AA,
                            monolish::tensor::tensor_Dense<T> &BB,
                            monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tq>
    ...
    test_send_tt_max_view_core2(const size_t M, const size_t N, const size_t L,
                                ...
                                monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                                std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  if (!test_send_tt_max_view_core3<T, MAT_A, decltype(std::get<J>(Bs)), 0,
                                   Tq...>(M, N, L, tol, AA, BB, CC, A,
                                          std::get<J>(Bs), Cs)) {
    return false;
  }
  return test_send_tt_max_view_core2<T, MAT_A, J + 1, Tq...>(M, N, L, tol, AA,
                                                             BB, CC, A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_tt_max_view_core1(const size_t M, const size_t N, const size_t L,
                            double tol, monolish::tensor::tensor_Dense<T> &AA,
                            monolish::tensor::tensor_Dense<T> &BB,
...
template <typename T, std::size_t I = 0, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type test_send_tt_max_view_core1(
  ...
  if (!test_send_tt_max_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
...
template <typename T>
bool test_send_tt_max_view(const size_t M, const size_t N, const size_t L,
                           double tol) {
  monolish::tensor::tensor_Dense<T> seed1({M, N, L}, 1.0, 2.0);
  monolish::tensor::tensor_Dense<T> seed2({M, N, L}, 1.0, 2.0);
  monolish::tensor::tensor_COO<T> seedA(seed1), seedB(seed2);
  ...
  return test_send_tt_max_view_core1<T, 0, T1, T2, T3, T4>(M, N, L, tol, AA, BB,
                                                           CC, As, Bs, Cs);
}

template <typename T, typename MAT_A, typename MAT_B, typename MAT_C>
bool test_tt_max_core(const size_t M, const size_t N, const size_t L,
                      double tol, monolish::tensor::tensor_Dense<T> &AA,
                      monolish::tensor::tensor_Dense<T> &BB,
...
bool test_tt_max(const size_t M, const size_t N, const size_t L, double tol) {
  ...
  return test_tt_max_core<T, MAT_A, MAT_B, MAT_C>(M, N, L, tol, AA, BB, CC, A,
                                                  B, C);
}
...
template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
          typename... Tr>
inline typename std::enable_if<K == sizeof...(Tr), bool>::type
test_tt_max_view_core3(const size_t M, const size_t N, const size_t L,
                       double tol, monolish::tensor::tensor_Dense<T> &AA,
                       monolish::tensor::tensor_Dense<T> &BB,
                       monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                       MAT_B &B, std::tuple<Tr...> &Cs) {
...
template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
  ...
  A = AA;
  B = BB;
  if (!test_tt_max_core<T, MAT_A, MAT_B, decltype(std::get<K>(Cs))>(
          M, N, L, tol, AA, BB, CC, A, B, std::get<K>(Cs))) {
    return false;
  }
  return test_tt_max_view_core3<T, MAT_A, MAT_B, K + 1, Tr...>(
      M, N, L, tol, AA, BB, CC, A, B, Cs);
}

template <typename T, typename MAT_A, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_tt_max_view_core2(const size_t M, const size_t N, const size_t L,
                       double tol, monolish::tensor::tensor_Dense<T> &AA,
                       monolish::tensor::tensor_Dense<T> &BB,
                       monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                       std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tq>
    ...
    test_tt_max_view_core2(const size_t M, const size_t N, const size_t L,
                           ...
                           monolish::tensor::tensor_Dense<T> &CC, MAT_A &A,
                           std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  if (!test_tt_max_view_core3<T, MAT_A, decltype(std::get<J>(Bs)), 0, Tq...>(
          M, N, L, tol, AA, BB, CC, A, std::get<J>(Bs), Cs)) {
    return false;
  }
  return test_tt_max_view_core2<T, MAT_A, J + 1, Tq...>(M, N, L, tol, AA, BB,
                                                        CC, A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_tt_max_view_core1(const size_t M, const size_t N, const size_t L,
                       double tol, monolish::tensor::tensor_Dense<T> &AA,
                       monolish::tensor::tensor_Dense<T> &BB,
                       monolish::tensor::tensor_Dense<T> &CC,
                       std::tuple<Tp...> &As, std::tuple<Tp...> &Bs,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_tt_max_view_core1(const size_t M, const size_t N, const size_t L,
                           ...
                           std::tuple<Tp...> &As, std::tuple<Tp...> &Bs,
                           std::tuple<Tp...> &Cs) {
  if (!test_tt_max_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, L, tol, AA, BB, CC, std::get<I>(As), Bs, Cs)) {
    return false;
  }
  return true;
...
template <typename T>
bool test_tt_max_view(const size_t M, const size_t N, const size_t L,
                      double tol) {
  monolish::tensor::tensor_Dense<T> seed1({M, N, L}, 1.0, 2.0);
  monolish::tensor::tensor_Dense<T> seed2({M, N, L}, 1.0, 2.0);
  monolish::tensor::tensor_COO<T> seedA(seed1), seedB(seed2);
  ...
  return test_tt_max_view_core1<T, 0, T1, T2, T3, T4>(M, N, L, tol, AA, BB, CC,
                                                      As, Bs, Cs);
}
