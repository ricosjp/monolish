...
bool test(const size_t M, const size_t N, const size_t K, double tol, int iter, int check_ans){
    ...
    monolish::matrix::COO<T> seedA = monolish::util::random_structure_matrix<T>(M, K, nnzrow, 1.0);
    monolish::matrix::COO<T> seedB = monolish::util::random_structure_matrix<T>(K, N, nnzrow, 1.0);
    monolish::matrix::COO<T> seedC = monolish::util::random_structure_matrix<T>(M, N, nnzrow, 1.0);
    ...
    if(check_ans == 1){
        ...
        if(ans_check<T>(resultC.val.data(), ansC.val.data(), ansC.get_nnz(), tol) == false){
            return false;
        };
...
int main(int argc, char** argv){
    ...
    if(argc!=10){
        std::cout << "error $1: precision (double or float) \
            ...
            $5: M, $6: N, $7: K, $8: iter, $9: error check (1/0)" << std::endl;
        return 1;
    }
    ...
    if(strcmp(argv[1],"double")==0){
        if( (strcmp(argv[2],"Dense") == 0) && (strcmp(argv[3],"Dense") == 0) && (strcmp(argv[4],"Dense") ==0)){
            if( test<
                    monolish::matrix::Dense<double>, 
                    monolish::matrix::Dense<double>, 
                    monolish::matrix::Dense<double>,
                    double>
                    (M, N, K, 1.0e-6, iter, check_ans) == false){ return 1; }
        }

        if( (strcmp(argv[2],"CRS") == 0) && (strcmp(argv[3],"Dense") == 0) && (strcmp(argv[4],"Dense") ==0)){
            if( test<
