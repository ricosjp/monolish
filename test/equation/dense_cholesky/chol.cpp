...
template <typename T>
bool Cholesky_test(const char *file, const int check_ans, const T tol) {

  monolish::equation::Cholesky<monolish::matrix::Dense<T>, T> Cholesky_solver;
...
int main(int argc, char **argv) {
  ...
  if (Cholesky_test<double>(file, check_ans, 1.0e-8) == false) {
    return 1;
  }
  if (Cholesky_test<float>(file, check_ans, 1.0e-5) == false) {
    return 1;
  }
