...
bool test_send_sm_max_core(const size_t M, const size_t N, double tol,
...
bool test_send_sm_max(const size_t M, const size_t N, double tol) {
  ...
  return test_send_sm_max_core(M, N, tol, AA, CC, A, C);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_send_sm_max_view_core2(const size_t M, const size_t N, double tol,
                            monolish::matrix::Dense<T> &AA,
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    ...
    test_send_sm_max_view_core2(const size_t M, const size_t N, double tol,
                                ...
                                std::tuple<Tq...> &Bs) {
  A = AA;
  if (!test_send_sm_max_core<T, MAT1, decltype(std::get<J>(Bs))>(
          M, N, tol, AA, CC, A, std::get<J>(Bs))) {
    return false;
  }
  return test_send_sm_max_view_core2<T, MAT1, J + 1, Tq...>(M, N, tol, AA, CC,
                                                            A, Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_sm_max_view_core1(const size_t M, const size_t N, double tol,
                            monolish::matrix::Dense<T> &AA,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_send_sm_max_view_core1(const size_t M, const size_t N, double tol,
                                ...
                                monolish::matrix::Dense<T> &CC,
                                std::tuple<Tp...> &As, std::tuple<Tp...> &Bs) {
  if (!test_send_sm_max_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, tol, AA, CC, std::get<I>(As), Bs)) {
    return false;
  }
  return test_send_sm_max_view_core1<T, I + 1, Tp...>(M, N, tol, AA, CC, As,
                                                      Bs);
}

template <typename T>
bool test_send_sm_max_view(const size_t M, const size_t N, double tol) {
  ...
  return test_send_sm_max_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, CC,
                                                           As, Bs);
}

template <typename T, typename MAT_A, typename MAT_C>
bool test_sm_max_core(const size_t M, const size_t N, double tol,
                      monolish::matrix::Dense<T> &AA,
                      monolish::matrix::Dense<T> &CC, MAT_A &A, MAT_C &C) {
  T alpha = 1.5;
  ans_sm_max(AA, alpha, CC);
...
bool test_sm_max(const size_t M, const size_t N, double tol) {
  ...
  return test_sm_max_core(M, N, tol, AA, CC, A, C);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_sm_max_view_core2(const size_t M, const size_t N, double tol,
                       monolish::matrix::Dense<T> &AA,
                       monolish::matrix::Dense<T> &CC, MAT1 &A,
                       std::tuple<Tq...> &Bs) {
  return true;
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    ...
    test_sm_max_view_core2(const size_t M, const size_t N, double tol,
                           ...
                           std::tuple<Tq...> &Bs) {
  A = AA;
  if (!test_sm_max_core<T, MAT1, decltype(std::get<J>(Bs))>(
          M, N, tol, AA, CC, A, std::get<J>(Bs))) {
    return false;
  }
  return test_sm_max_view_core2<T, MAT1, J + 1, Tq...>(M, N, tol, AA, CC, A,
                                                       Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_sm_max_view_core1(const size_t M, const size_t N, double tol,
                       monolish::matrix::Dense<T> &AA,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_sm_max_view_core1(const size_t M, const size_t N, double tol,
                           ...
                           monolish::matrix::Dense<T> &CC,
                           std::tuple<Tp...> &As, std::tuple<Tp...> &Bs) {
  if (!test_sm_max_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, tol, AA, CC, std::get<I>(As), Bs)) {
    return false;
  }
  return test_sm_max_view_core1<T, I + 1, Tp...>(M, N, tol, AA, CC, As, Bs);
}

template <typename T>
bool test_sm_max_view(const size_t M, const size_t N, double tol) {
  monolish::matrix::Dense<T> seed1(M, N, 1.0, 2.0);
  monolish::matrix::COO<T> seedA(seed1);
  monolish::matrix::Dense<T> AA(seedA), CC(seedA);
  ...
  return test_sm_max_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, CC, As,
                                                      Bs);
}
