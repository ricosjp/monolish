...
template <typename T>
void ans_mm_min(const monolish::matrix::Dense<T> &A,
...
bool test_send_mm_min_core(const size_t M, const size_t N, double tol,
...
bool test_send_mm_min(const size_t M, const size_t N, double tol) {
  ...
  return test_send_mm_min_core(M, N, tol, AA, BB, CC, A, B, C);
}

template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
          typename... Tr>
inline typename std::enable_if<K == sizeof...(Tr), bool>::type
test_send_mm_min_view_core3(const size_t M, const size_t N, double tol,
                            monolish::matrix::Dense<T> &AA,
                            monolish::matrix::Dense<T> &BB,
                            monolish::matrix::Dense<T> &CC, MAT_A &A, MAT_B &B,
                            std::tuple<Tr...> &Cs) {
...
template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
  ...
  A = AA;
  B = BB;
  if (!test_send_mm_min_core<T, MAT_A, MAT_B, decltype(std::get<K>(Cs))>(
          M, N, tol, AA, BB, CC, A, B, std::get<K>(Cs))) {
    return false;
  }
  return test_send_mm_min_view_core3<T, MAT_A, MAT_B, K + 1, Tr...>(
...
test_send_mm_min_view_core2(const size_t M, const size_t N, double tol,
                            monolish::matrix::Dense<T> &AA,
                            monolish::matrix::Dense<T> &BB,
                            monolish::matrix::Dense<T> &CC, MAT_A &A,
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tq>
    ...
    test_send_mm_min_view_core2(const size_t M, const size_t N, double tol,
                                ...
                                monolish::matrix::Dense<T> &CC, MAT_A &A,
                                std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  if (!test_send_mm_min_view_core3<T, MAT_A, decltype(std::get<J>(Bs)), 0,
                                   Tq...>(M, N, tol, AA, BB, CC, A,
                                          std::get<J>(Bs), Cs)) {
    return false;
  }
  return test_send_mm_min_view_core2<T, MAT_A, J + 1, Tq...>(M, N, tol, AA, BB,
                                                             CC, A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_mm_min_view_core1(const size_t M, const size_t N, double tol,
                            monolish::matrix::Dense<T> &AA,
                            monolish::matrix::Dense<T> &BB,
...
template <typename T, std::size_t I = 0, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type test_send_mm_min_view_core1(
  ...
  if (!test_send_mm_min_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
...
template <typename T>
bool test_send_mm_min_view(const size_t M, const size_t N, double tol) {
  monolish::matrix::Dense<T> seed1(M, N, 1.0, 2.0);
  monolish::matrix::Dense<T> seed2(M, N, 1.0, 2.0);
  monolish::matrix::COO<T> seedA(seed1), seedB(seed2);
  ...
  return test_send_mm_min_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, BB,
                                                           CC, As, Bs, Cs);
}

template <typename T, typename MAT_A, typename MAT_B, typename MAT_C>
bool test_mm_min_core(const size_t M, const size_t N, double tol,
                      monolish::matrix::Dense<T> &AA,
                      monolish::matrix::Dense<T> &BB,
                      monolish::matrix::Dense<T> &CC, MAT_A &A, MAT_B &B,
                      MAT_C &C) {
...
template <typename MAT_A, typename MAT_B, typename MAT_C, typename T>
bool test_mm_min(const size_t M, const size_t N, double tol) {
  monolish::matrix::Dense<T> seed1(M, N, 1.0, 2.0);
  monolish::matrix::Dense<T> seed2(M, N, 1.0, 2.0);
  monolish::matrix::COO<T> seedA(seed1), seedB(seed2);
  ...
  return test_mm_min_core(M, N, tol, AA, BB, CC, A, B, C);
}

template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
          typename... Tr>
inline typename std::enable_if<K == sizeof...(Tr), bool>::type
test_mm_min_view_core3(const size_t M, const size_t N, double tol,
                       monolish::matrix::Dense<T> &AA,
                       monolish::matrix::Dense<T> &BB,
                       monolish::matrix::Dense<T> &CC, MAT_A &A, MAT_B &B,
                       std::tuple<Tr...> &Cs) {
...
template <typename T, typename MAT_A, typename MAT_B, std::size_t K = 0,
  ...
  A = AA;
  B = BB;
  if (!test_mm_min_core<T, MAT_A, MAT_B, decltype(std::get<K>(Cs))>(
          M, N, tol, AA, BB, CC, A, B, std::get<K>(Cs))) {
    return false;
  }
  return test_mm_min_view_core3<T, MAT_A, MAT_B, K + 1, Tr...>(
      M, N, tol, AA, BB, CC, A, B, Cs);
}

template <typename T, typename MAT_A, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_mm_min_view_core2(const size_t M, const size_t N, double tol,
                       monolish::matrix::Dense<T> &AA,
                       monolish::matrix::Dense<T> &BB,
                       monolish::matrix::Dense<T> &CC, MAT_A &A,
                       std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tq>
    ...
    test_mm_min_view_core2(const size_t M, const size_t N, double tol,
                           ...
                           monolish::matrix::Dense<T> &CC, MAT_A &A,
                           std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  if (!test_mm_min_view_core3<T, MAT_A, decltype(std::get<J>(Bs)), 0, Tq...>(
          M, N, tol, AA, BB, CC, A, std::get<J>(Bs), Cs)) {
    return false;
  }
  return test_mm_min_view_core2<T, MAT_A, J + 1, Tq...>(M, N, tol, AA, BB, CC,
                                                        A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_mm_min_view_core1(const size_t M, const size_t N, double tol,
                       monolish::matrix::Dense<T> &AA,
                       monolish::matrix::Dense<T> &BB,
                       monolish::matrix::Dense<T> &CC, std::tuple<Tp...> &As,
                       std::tuple<Tp...> &Bs, std::tuple<Tp...> &Cs) {
...
template <typename T, std::size_t I = 0, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type test_mm_min_view_core1(
        const size_t M, const size_t N, double tol,
        monolish::matrix::Dense<T> &AA, monolish::matrix::Dense<T> &BB,
        monolish::matrix::Dense<T> &CC, std::tuple<Tp...> &As,
        std::tuple<Tp...> &Bs, std::tuple<Tp...> &Cs) {
  if (!test_mm_min_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, tol, AA, BB, CC, std::get<I>(As), Bs, Cs)) {
    return false;
  }
  return true;
...
template <typename T>
bool test_mm_min_view(const size_t M, const size_t N, double tol) {
  monolish::matrix::Dense<T> seed1(M, N, 1.0, 2.0);
  monolish::matrix::Dense<T> seed2(M, N, 1.0, 2.0);
  monolish::matrix::COO<T> seedA(seed1), seedB(seed2);
  monolish::matrix::Dense<T> AA(seedA), BB(seedB), CC(seedA);
  ...
  return test_mm_min_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, BB, CC, As,
                                                      Bs, Cs);
}
