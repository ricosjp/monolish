...
bool test_send_mmax_core(const size_t M, const size_t N, double tol,
...
bool test_send_mmax(const size_t M, const size_t N, double tol) {
  ...
  return test_send_mmax_core(M, N, tol, AA, A);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_mmax_view_core1(const size_t M, const size_t N, double tol,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_send_mmax_view_core1(const size_t M, const size_t N, double tol,
                              ...
                              std::tuple<Tp...> &As) {
  std::get<I>(As) = AA;
  if (!test_send_mmax_core(M, N, tol, AA, std::get<I>(As))) {
    return false;
  }
  return test_send_mmax_view_core1<T, I + 1, Tp...>(M, N, tol, AA, As);
}
...
bool test_send_mmax_view(const size_t M, const size_t N, double tol) {
  ...
  return test_send_mmax_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, As);
}

template <typename T, typename MAT1>
bool test_mmax_core(const size_t M, const size_t N, double tol,
                    monolish::matrix::Dense<T> &AA, MAT1 &A) {
  T ans = ans_max(AA);
...
bool test_mmax(const size_t M, const size_t N, double tol) {
  ...
  return test_mmax_core(M, N, tol, AA, A);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_mmax_view_core1(const size_t M, const size_t N, double tol,
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_mmax_view_core1(const size_t M, const size_t N, double tol,
                         ...
                         std::tuple<Tp...> &As) {
  std::get<I>(As) = AA;
  if (!test_mmax_core(M, N, tol, AA, std::get<I>(As))) {
    return false;
  }
  return test_mmax_view_core1<T, I + 1, Tp...>(M, N, tol, AA, As);
}
...
bool test_mmax_view(const size_t M, const size_t N, double tol) {
  ...
  return test_mmax_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, As);
}
