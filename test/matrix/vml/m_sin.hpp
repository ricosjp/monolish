...
template <typename MAT> void ans_sin(MAT &A) {
...
bool test_send_msin_core(const size_t M, const size_t N, double tol,
...
bool test_send_msin(const size_t M, const size_t N, double tol) {
  ...
  return test_send_msin_core<T, MAT, MAT>(M, N, tol, AA, A, A);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_send_msin_view_core2(const size_t M, const size_t N, double tol,
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    ...
    test_send_msin_view_core2(const size_t M, const size_t N, double tol,
                              ...
                              std::tuple<Tq...> &Bs) {
  A = AA;
  if (!test_send_msin_core<T, MAT1, decltype(std::get<J>(Bs))>(
          M, N, tol, AA, A, std::get<J>(Bs))) {
    return false;
  }
  return test_send_msin_view_core2<T, MAT1, J + 1, Tq...>(M, N, tol, AA, A, Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_msin_view_core1(const size_t M, const size_t N, double tol,
...
template <typename T, std::size_t I = 0, typename... Tp>
  ...
  if (!test_send_msin_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, tol, AA, std::get<I>(As), Bs)) {
    return false;
  }
  return test_send_msin_view_core1<T, I + 1, Tp...>(M, N, tol, AA, As, Bs);
}
...
bool test_send_msin_view(const size_t M, const size_t N, double tol) {
  ...
  return test_send_msin_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, As, Bs);
}

template <typename T, typename MAT1, typename MAT2>
bool test_msin_core(const size_t M, const size_t N, double tol,
                    monolish::matrix::Dense<T> &AA, MAT1 &A, MAT2 &B) {
  ans_sin(AA);
  monolish::vml::sin(A, B);
  monolish::matrix::Dense<T> resultA(B);
...
bool test_msin(const size_t M, const size_t N, double tol) {
  ...
  return test_msin_core<T, MAT, MAT>(M, N, tol, AA, A, A);
}

template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_msin_view_core2(const size_t M, const size_t N, double tol,
                     monolish::matrix::Dense<T> &AA, MAT1 &A,
                     std::tuple<Tq...> &Bs) {
  return true;
}
...
template <typename T, typename MAT1, std::size_t J = 0, typename... Tq>
    ...
    test_msin_view_core2(const size_t M, const size_t N, double tol,
                         ...
                         std::tuple<Tq...> &Bs) {
  A = AA;
  if (!test_msin_core<T, MAT1, decltype(std::get<J>(Bs))>(M, N, tol, AA, A,
                                                          std::get<J>(Bs))) {
    return false;
  }
  return test_msin_view_core2<T, MAT1, J + 1, Tq...>(M, N, tol, AA, A, Bs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_msin_view_core1(const size_t M, const size_t N, double tol,
                     monolish::matrix::Dense<T> &AA, std::tuple<Tp...> &As,
                     std::tuple<Tp...> &Bs) {
  return true;
}
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_msin_view_core1(const size_t M, const size_t N, double tol,
                         monolish::matrix::Dense<T> &AA, std::tuple<Tp...> &As,
                         std::tuple<Tp...> &Bs) {
  if (!test_msin_view_core2<T, decltype(std::get<I>(As)), 0, Tp...>(
          M, N, tol, AA, std::get<I>(As), Bs)) {
    return false;
  }
  return test_msin_view_core1<T, I + 1, Tp...>(M, N, tol, AA, As, Bs);
}
...
bool test_msin_view(const size_t M, const size_t N, double tol) {
  ...
  return test_msin_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, As, Bs);
}
