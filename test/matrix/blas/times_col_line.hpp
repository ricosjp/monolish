...
bool test_send_times_col_line_core(const size_t M, const size_t N, VEC& x, double tol) {
  size_t nnzrow = 27;
  if (nnzrow < N) {
    nnzrow = 27;
  } else {
...
bool test_send_times_col_line(const size_t M, const size_t N,
                        double tol) {
  monolish::vector<T> vec(M, 0.0, 1.0, test_random_engine());
  return test_send_times_col_line_core<MAT, monolish::vector<T>, T>(M, N, vec, tol);
}

template <typename MAT, typename VEC, typename T>
bool test_times_col_line_core(const size_t M, const size_t N, VEC& x, double tol) {
  size_t nnzrow = 27;
  if (nnzrow < N) {
    nnzrow = 27;
  } else {
...
template <typename MAT, typename T>
bool test_times_col_line(const size_t M, const size_t N,
                        double tol) {
  monolish::vector<T> vec(M, 0.0, 1.0, test_random_engine());
...
bool test_send_times_col_line_view(const size_t M, const size_t N, double tol){
  ...
  return test_send_times_col_line_core<MAT, monolish::view1D<U, T>, T>(M, N, vec, tol);
}
...
bool test_times_col_line_view(const size_t M, const size_t N, double tol){
  U x(M, 0.0, 1.0);
  monolish::view1D<U, T> vec(x, 0, M);
  return test_times_col_line_core<MAT, monolish::view1D<U, T>, T>(M, N, vec, tol);
}
