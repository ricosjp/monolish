...
bool test_send_times_row_line_core(const size_t M, const size_t N, VEC &x,
                                   double tol) {
  size_t nnzrow = 27;
  if (nnzrow < N) {
    nnzrow = 27;
...
bool test_send_times_row_line(const size_t M, const size_t N, double tol) {
  ...
  vec.send();
  return test_send_times_row_line_core<MAT, monolish::vector<T>, T>(M, N, vec,
                                                                    tol);
}

template <typename MAT, typename VEC, typename T>
bool test_times_row_line_core(const size_t M, const size_t N, VEC &x,
                              double tol) {
  size_t nnzrow = 27;
  if (nnzrow < N) {
    nnzrow = 27;
...
bool test_times_row_line(const size_t M, const size_t N, double tol) {
  monolish::vector<T> vec(N, 0.0, 1.0, test_random_engine());
  return test_times_row_line_core<MAT, monolish::vector<T>, T>(M, N, vec, tol);
}

template <typename MAT, typename T>
bool test_send_times_row_line_view_core(const size_t M, const size_t N,
                                        double tol) {
  return true;
}
...
bool test_send_times_row_line_view_core(const size_t M, const size_t N,
  ...
  if (!test_send_times_row_line_core<MAT, monolish::view1D<U, T>, T>(M, N, vec,
                                                                     tol)) {
    return false;
  }
  return test_send_times_row_line_view_core<MAT, T, ARGS...>(M, N, tol,
                                                             args...);
}

template <typename MAT, typename T>
bool test_send_times_row_line_view(const size_t M, const size_t N, double tol) {
  ...
  return test_send_times_row_line_view_core<MAT, T, T1, T2, T3>(M, N, tol, x1,
                                                                x2, x3);
}

template <typename MAT, typename T>
bool test_times_row_line_view_core(const size_t M, const size_t N, double tol) {
  return true;
}
...
bool test_times_row_line_view_core(const size_t M, const size_t N, double tol,
                                   U x, ARGS... args) {
  monolish::view1D<U, T> vec(x, N / 2, N);
  if (!test_times_row_line_core<MAT, monolish::view1D<U, T>, T>(M, N, vec,
                                                                tol)) {
    return false;
  }
  return test_times_row_line_view_core<MAT, T, ARGS...>(M, N, tol, args...);
}

template <typename MAT, typename T>
bool test_times_row_line_view(const size_t M, const size_t N, double tol) {
  using T1 = monolish::vector<T>;
  using T2 = monolish::matrix::Dense<T>;
  using T3 = monolish::tensor::tensor_Dense<T>;
  T1 x1(2 * N, 0.0, 1.0);
  ...
  return test_times_row_line_view_core<MAT, T, T1, T2, T3>(M, N, tol, x1, x2,
                                                           x3);
}
