...
void ans_times_row(const monolish::matrix::Dense<T> &A,
                   const VEC &mx,
...
bool test_send_times_row_core(const size_t M, const size_t N, VEC& x, double tol) {
  size_t nnzrow = 27;
  if (nnzrow < N) {
    nnzrow = 27;
  } else {
...
bool test_send_times_row(const size_t M, const size_t N,
                        double tol) {
...
template <typename MAT, typename T>
bool test_times_row(const size_t M, const size_t N,
                        double tol) {
  monolish::vector<T> vec(N, 0.0, 1.0, test_random_engine());
...
bool test_send_times_row_view(const size_t M, const size_t N, double tol){
  ...
  return test_send_times_row_core<MAT, monolish::view1D<U, T>, T>(M, N, vec, tol);
}
