...
bool test_send_matvec(const size_t M, const size_t N, double tol) {
  ...
  return test_send_matvec_core(M, N, tol, AA, x, A, x, y);
}

template <typename T, typename MAT, typename VEC_A, std::size_t K = 0,
          typename... Tq>
inline typename std::enable_if<K == sizeof...(Tq), bool>::type
test_send_matvec_view_dense_core3(const size_t M, const size_t N, double tol,
                                  monolish::matrix::Dense<T> &AA,
                                  monolish::vector<T> &xx, MAT &A, VEC_A &x,
                                  std::tuple<Tq...> &Cs) {
  return true;
...
template <typename T, typename MAT, typename VEC_A, std::size_t K = 0,
    ...
    K<sizeof...(Tq), bool>::type test_send_matvec_view_dense_core3(
        const size_t M, const size_t N, double tol,
        monolish::matrix::Dense<T> &AA, monolish::vector<T> &xx, MAT &A,
        VEC_A &x, std::tuple<Tq...> &Cs) {
  A = AA;
  x = xx;
  if (!test_send_matvec_core(M, N, tol, AA, xx, A, x, std::get<K>(Cs))) {
    return false;
  }
  return test_send_matvec_view_dense_core3<T, MAT, VEC_A, K + 1, Tq...>(
...
test_send_matvec_view_dense_core2(const size_t M, const size_t N, double tol,
                                  monolish::matrix::Dense<T> &AA,
...
template <typename T, typename MAT, std::size_t J = 0, typename... Tq>
    inline typename std::enable_if <
    J<sizeof...(Tq), bool>::type test_send_matvec_view_dense_core2(
        const size_t M, const size_t N, double tol,
        monolish::matrix::Dense<T> &AA, monolish::vector<T> &xx, MAT &A,
        std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  if (!test_send_matvec_view_dense_core3(M, N, tol, AA, xx, A, std::get<J>(Bs),
                                         Cs)) {
    return false;
  }
  return test_send_matvec_view_dense_core2<T, MAT, J + 1, Tq...>(M, N, tol, AA,
                                                                 xx, A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_matvec_view_dense_core1(const size_t M, const size_t N, double tol,
                                  monolish::matrix::Dense<T> &AA,
...
template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type test_send_matvec_view_dense_core1(
        const size_t M, const size_t N, double tol,
        monolish::matrix::Dense<T> &AA, monolish::vector<T> &xx,
        std::tuple<Tp...> &As, VECS &Bs, VECS &Cs) {
  if (!test_send_matvec_view_dense_core2(M, N, tol, AA, xx, std::get<I>(As), Bs,
                                         Cs)) {
    return false;
  }
  return test_send_matvec_view_dense_core1<T, I + 1, VECS, Tp...>(
      M, N, tol, AA, xx, As, Bs, Cs);
}

template <typename T>
bool test_send_matvec_view_dense(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  monolish::matrix::Dense<T> AA(seedA);
  ...
  return test_send_matvec_view_dense_core1(M, N, tol, AA, xx, As, Bs, Cs);
}

template <typename T, typename VEC_A, std::size_t K = 0, typename... Tq>
inline typename std::enable_if<K == sizeof...(Tq), bool>::type
test_send_matvec_view_crs_core2(const size_t M, const size_t N, double tol,
...
template <typename T, typename VEC_A, std::size_t K = 0, typename... Tq>
  ...
  return test_send_matvec_view_crs_core2<T, VEC_A, K + 1, Tq...>(
      M, N, tol, seedA, xx, x, Cs);
}

template <typename T, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_send_matvec_view_crs_core1(const size_t M, const size_t N, double tol,
                                monolish::matrix::COO<T> &seedA,
...
template <typename T, std::size_t J = 0, typename... Tq>
    inline typename std::enable_if <
    J<sizeof...(Tq), bool>::type test_send_matvec_view_crs_core1(
        const size_t M, const size_t N, double tol,
        monolish::matrix::COO<T> &seedA, monolish::vector<T> &xx,
        std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  if (!test_send_matvec_view_crs_core2(M, N, tol, seedA, xx, std::get<J>(Bs),
                                       Cs)) {
    return false;
  }
  return test_send_matvec_view_crs_core1<T, J + 1, Tq...>(M, N, tol, seedA, xx,
                                                          Bs, Cs);
}

template <typename T>
bool test_send_matvec_view_crs(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  ...
  return test_send_matvec_view_crs_core1(M, N, tol, seedA, xx, Bs, Cs);
}
...
template <typename T, typename MAT, typename VEC_A, typename VEC_B>
bool test_matvec_core(const size_t M, const size_t N, double tol,
                      monolish::matrix::Dense<T> &AA, monolish::vector<T> &xx,
...
bool test_matvec(const size_t M, const size_t N, double tol) {
  ...
  return test_matvec_core(M, N, tol, AA, x, A, x, y);
}

template <typename T, typename MAT, typename VEC_A, std::size_t K = 0,
          typename... Tq>
inline typename std::enable_if<K == sizeof...(Tq), bool>::type
test_matvec_view_dense_core3(const size_t M, const size_t N, double tol,
                             monolish::matrix::Dense<T> &AA,
                             monolish::vector<T> &xx, MAT &A, VEC_A &x,
                             std::tuple<Tq...> &Cs) {
  return true;
...
template <typename T, typename MAT, typename VEC_A, std::size_t K = 0,
  ...
  A = AA;
  x = xx;
  if (!test_matvec_core(M, N, tol, AA, xx, A, x, std::get<K>(Cs))) {
    return false;
  }
  return test_matvec_view_dense_core3<T, MAT, VEC_A, K + 1, Tq...>(
      M, N, tol, AA, xx, A, x, Cs);
}

template <typename T, typename MAT, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_matvec_view_dense_core2(const size_t M, const size_t N, double tol,
                             monolish::matrix::Dense<T> &AA,
                             monolish::vector<T> &xx, MAT &A,
                             std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  return true;
...
template <typename T, typename MAT, std::size_t J = 0, typename... Tq>
    ...
    test_matvec_view_dense_core2(const size_t M, const size_t N, double tol,
                                 ...
                                 monolish::vector<T> &xx, MAT &A,
                                 std::tuple<Tq...> &Bs, std::tuple<Tq...> &Cs) {
  if (!test_matvec_view_dense_core3(M, N, tol, AA, xx, A, std::get<J>(Bs),
                                    Cs)) {
    return false;
  }
  return test_matvec_view_dense_core2<T, MAT, J + 1, Tq...>(M, N, tol, AA, xx,
                                                            A, Bs, Cs);
}

template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_matvec_view_dense_core1(const size_t M, const size_t N, double tol,
                             monolish::matrix::Dense<T> &AA,
...
template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
    ...
    test_matvec_view_dense_core1(const size_t M, const size_t N, double tol,
                                 ...
                                 monolish::vector<T> &xx, std::tuple<Tp...> &As,
                                 VECS &Bs, VECS &Cs) {
  if (!test_matvec_view_dense_core2(M, N, tol, AA, xx, std::get<I>(As), Bs,
                                    Cs)) {
    return false;
  }
  return test_matvec_view_dense_core1<T, I + 1, VECS, Tp...>(M, N, tol, AA, xx,
                                                             As, Bs, Cs);
}

template <typename T>
bool test_matvec_view_dense(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  monolish::matrix::Dense<T> AA(seedA);
  monolish::vector<T> xx(AA.get_col(), 0.0, 1.0, test_random_engine());
  ...
  return test_matvec_view_dense_core1(M, N, tol, AA, xx, As, Bs, Cs);
}

template <typename T, typename VEC_A, std::size_t K = 0, typename... Tq>
inline typename std::enable_if<K == sizeof...(Tq), bool>::type
test_matvec_view_crs_core2(const size_t M, const size_t N, double tol,
                           monolish::matrix::COO<T> &seedA,
                           monolish::vector<T> &xx, VEC_A &x,
                           std::tuple<Tq...> &Cs) {
  return true;
...
template <typename T, typename VEC_A, std::size_t K = 0, typename... Tq>
  ...
  return test_matvec_view_crs_core2<T, VEC_A, K + 1, Tq...>(M, N, tol, seedA,
                                                            xx, x, Cs);
}

template <typename T, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_matvec_view_crs_core1(const size_t M, const size_t N, double tol,
                           monolish::matrix::COO<T> &seedA,
                           monolish::vector<T> &xx, std::tuple<Tq...> &Bs,
                           std::tuple<Tq...> &Cs) {
  return true;
...
template <typename T, std::size_t J = 0, typename... Tq>
    ...
    test_matvec_view_crs_core1(const size_t M, const size_t N, double tol,
                               ...
                               monolish::vector<T> &xx, std::tuple<Tq...> &Bs,
                               std::tuple<Tq...> &Cs) {
  if (!test_matvec_view_crs_core2(M, N, tol, seedA, xx, std::get<J>(Bs), Cs)) {
    return false;
  }
  return test_matvec_view_crs_core1<T, J + 1, Tq...>(M, N, tol, seedA, xx, Bs,
                                                     Cs);
}

template <typename T>
bool test_matvec_view_crs(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  monolish::vector<T> xx(seedA.get_col(), 0.0, 1.0, test_random_engine());
  ...
  return test_matvec_view_crs_core1(M, N, tol, seedA, xx, Bs, Cs);
}

template <typename MAT, typename T>
bool test_matvec_linearoperator(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
