...
bool test_send_times_col_core(const size_t M, const size_t N, double tol,
                              monolish::matrix::Dense<T> &AA,
                              monolish::vector<T> &vec,
                              monolish::matrix::Dense<T> &CC, MAT_A &A, VEC &x,
                              MAT_C &C) {
  ans_times_col(AA, vec, CC);
...
bool test_send_times_col(const size_t M, const size_t N, double tol) {
  ...
  return test_send_times_col_core(M, N, tol, AA, x, CC, A, x, C);
}

template <typename T, typename MAT_A, typename VEC, std::size_t K = 0,
          typename... Tp>
inline typename std::enable_if<K == sizeof...(Tp), bool>::type
test_send_times_col_view_dense_core3(const size_t M, const size_t N, double tol,
                                     monolish::matrix::Dense<T> &AA,
                                     monolish::vector<T> &vec,
...
template <typename T, typename MAT_A, typename VEC, std::size_t K = 0,
    ...
    K<sizeof...(Tp), bool>::type test_send_times_col_view_dense_core3(
  ...
  A = AA;
  x = vec;
  if (!test_send_times_col_core(M, N, tol, AA, vec, CC, A, x,
                                std::get<K>(Cs))) {
    return false;
  }
  return test_send_times_col_view_dense_core3<T, MAT_A, VEC, K + 1, Tp...>(
...
test_send_times_col_view_dense_core2(const size_t M, const size_t N, double tol,
                                     monolish::matrix::Dense<T> &AA,
                                     monolish::vector<T> &vec,
                                     monolish::matrix::Dense<T> &CC, MAT_A &A,
...
template <typename T, typename MAT_A, std::size_t J = 0, typename MATS_C,
          typename... Tq>
    inline typename std::enable_if <
    J<sizeof...(Tq), bool>::type test_send_times_col_view_dense_core2(
        const size_t M, const size_t N, double tol,
        monolish::matrix::Dense<T> &AA, monolish::vector<T> &vec,
        monolish::matrix::Dense<T> &CC, MAT_A &A, std::tuple<Tq...> &xs,
        MATS_C &Cs) {
  if (!test_send_times_col_view_dense_core3(M, N, tol, AA, vec, CC, A,
                                            std::get<J>(xs), Cs)) {
    return false;
  }
  return test_send_times_col_view_dense_core2<T, MAT_A, J + 1, MATS_C, Tq...>(
      M, N, tol, AA, vec, CC, A, xs, Cs);
}

template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_times_col_view_dense_core1(const size_t M, const size_t N, double tol,
                                     monolish::matrix::Dense<T> &AA,
                                     monolish::vector<T> &vec,
...
template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type test_send_times_col_view_dense_core1(
        const size_t M, const size_t N, double tol,
        monolish::matrix::Dense<T> &AA, monolish::vector<T> &vec,
        monolish::matrix::Dense<T> &CC, std::tuple<Tp...> &As, VECS &xs,
        std::tuple<Tp...> &Cs) {
  if (!test_send_times_col_view_dense_core2(M, N, tol, AA, vec, CC,
                                            std::get<I>(As), xs, Cs)) {
    return false;
  }
  return test_send_times_col_view_dense_core1<T, I + 1, VECS, Tp...>(
      M, N, tol, AA, vec, CC, As, xs, Cs);
}

template <typename T>
bool test_send_times_col_view_dense(const size_t M, const size_t N,
                                    double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  ...
  return test_send_times_col_view_dense_core1(M, N, tol, AA, xx, CC, As, Bs,
                                              Cs);
}

template <typename T, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_send_times_col_view_crs_core1(const size_t M, const size_t N, double tol,
...
template <typename T, std::size_t J = 0, typename... Tq>
    inline typename std::enable_if <
    J<sizeof...(Tq), bool>::type test_send_times_col_view_crs_core1(
        const size_t M, const size_t N, double tol,
  ...
  std::get<J>(xs) = vec;
  if (!test_send_times_col_core(M, N, tol, AA, vec, CC, A, std::get<J>(xs),
                                C)) {
    return false;
  }
  return test_send_times_col_view_crs_core1<T, J + 1, Tq...>(M, N, tol, seedA,
                                                             vec, xs);
}

template <typename T>
bool test_send_times_col_view_crs(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  ...
  return test_send_times_col_view_crs_core1(M, N, tol, seedA, xx, Bs);
}

template <typename T, typename MAT_A, typename MAT_C, typename VEC>
bool test_times_col_core(const size_t M, const size_t N, double tol,
                         monolish::matrix::Dense<T> &AA,
                         monolish::vector<T> &vec,
                         monolish::matrix::Dense<T> &CC, MAT_A &A, VEC &x,
                         MAT_C &C) {
...
bool test_times_col(const size_t M, const size_t N, double tol) {
  ...
  return test_times_col_core(M, N, tol, AA, x, CC, A, x, C);
}

template <typename T, typename MAT_A, typename VEC, std::size_t K = 0,
          typename... Tp>
inline typename std::enable_if<K == sizeof...(Tp), bool>::type
test_times_col_view_dense_core3(const size_t M, const size_t N, double tol,
                                monolish::matrix::Dense<T> &AA,
                                monolish::vector<T> &vec,
                                monolish::matrix::Dense<T> &CC, MAT_A &A,
                                VEC &x, std::tuple<Tp...> &Cs) {
...
template <typename T, typename MAT_A, typename VEC, std::size_t K = 0,
  ...
  A = AA;
  x = vec;
  if (!test_times_col_core(M, N, tol, AA, vec, CC, A, x, std::get<K>(Cs))) {
    return false;
  }
  return test_times_col_view_dense_core3<T, MAT_A, VEC, K + 1, Tp...>(
      M, N, tol, AA, vec, CC, A, x, Cs);
}
...
template <typename T, typename MAT_A, std::size_t J = 0, typename MATS_C,
          typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_times_col_view_dense_core2(const size_t M, const size_t N, double tol,
                                monolish::matrix::Dense<T> &AA,
                                monolish::vector<T> &vec,
                                monolish::matrix::Dense<T> &CC, MAT_A &A,
                                std::tuple<Tq...> &x, MATS_C &Cs) {
...
template <typename T, typename MAT_A, std::size_t J = 0, typename MATS_C,
  ...
  if (!test_times_col_view_dense_core3(M, N, tol, AA, vec, CC, A,
                                       std::get<J>(xs), Cs)) {
    return false;
  }
  return test_times_col_view_dense_core2<T, MAT_A, J + 1, MATS_C, Tq...>(
      M, N, tol, AA, vec, CC, A, xs, Cs);
}

template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_times_col_view_dense_core1(const size_t M, const size_t N, double tol,
                                monolish::matrix::Dense<T> &AA,
                                monolish::vector<T> &vec,
                                monolish::matrix::Dense<T> &CC,
                                std::tuple<Tp...> &As, VECS &xs,
...
template <typename T, std::size_t I = 0, typename VECS, typename... Tp>
    inline typename std::enable_if <
    I<sizeof...(Tp), bool>::type test_times_col_view_dense_core1(
        const size_t M, const size_t N, double tol,
        monolish::matrix::Dense<T> &AA, monolish::vector<T> &vec,
        monolish::matrix::Dense<T> &CC, std::tuple<Tp...> &As, VECS &xs,
        std::tuple<Tp...> &Cs) {
  if (!test_times_col_view_dense_core2(M, N, tol, AA, vec, CC, std::get<I>(As),
                                       xs, Cs)) {
    return false;
  }
  return test_times_col_view_dense_core1<T, I + 1, VECS, Tp...>(
      M, N, tol, AA, vec, CC, As, xs, Cs);
}

template <typename T>
bool test_times_col_view_dense(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  monolish::matrix::Dense<T> AA(seedA), CC(seedA);
  monolish::vector<T> xx(AA.get_row(), 0.0, 1.0, test_random_engine());
  ...
  return test_times_col_view_dense_core1(M, N, tol, AA, xx, CC, As, Bs, Cs);
}

template <typename T, std::size_t J = 0, typename... Tq>
inline typename std::enable_if<J == sizeof...(Tq), bool>::type
test_times_col_view_crs_core1(const size_t M, const size_t N, double tol,
                              monolish::matrix::COO<T> &seedA,
...
template <typename T, std::size_t J = 0, typename... Tq>
  ...
  monolish::matrix::CRS<T> A(seedA), C(seedA);
  std::get<J>(xs) = vec;
  if (!test_times_col_core(M, N, tol, AA, vec, CC, A, std::get<J>(xs), C)) {
    return false;
  }
  return test_times_col_view_crs_core1<T, J + 1, Tq...>(M, N, tol, seedA, vec,
                                                        xs);
}

template <typename T>
bool test_times_col_view_crs(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  monolish::vector<T> xx(seedA.get_row(), 0.0, 1.0, test_random_engine());
  ...
  return test_times_col_view_crs_core1(M, N, tol, seedA, xx, Bs);
}
