...
bool test_send_scalar_adds_core(const size_t M, const size_t N, double tol,
...
bool test_send_scalar_adds(const size_t M, const size_t N, double tol) {
  ...
  return test_send_scalar_adds_core(M, N, tol, AA, CC, A, C);
}

template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
inline typename std::enable_if<J == sizeof...(Tp), bool>::type
test_send_scalar_adds_view_core2(const size_t M, const size_t N, double tol,
                                 monolish::matrix::Dense<T> &AA,
                                 monolish::matrix::Dense<T> &CC, MAT_A &A,
                                 std::tuple<Tp...> &Cs) {
  return true;
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
    ...
    test_send_scalar_adds_view_core2(const size_t M, const size_t N, double tol,
                                     ...
                                     std::tuple<Tp...> &Cs) {
  A = AA;
  if (!test_send_scalar_adds_core(M, N, tol, AA, CC, A, std::get<J>(Cs))) {
    return false;
  }
  return test_send_scalar_adds_view_core2<T, MAT_A, J + 1, Tp...>(M, N, tol, AA,
                                                                  CC, A, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_send_scalar_adds_view_core1(const size_t M, const size_t N, double tol,
                                 monolish::matrix::Dense<T> &AA,
...
template <typename T, std::size_t I = 0, typename... Tp>
  ...
  if (!test_send_scalar_adds_view_core2(M, N, tol, AA, CC, std::get<I>(As),
                                        Cs)) {
    return false;
  }
  return test_send_scalar_adds_view_core1<T, I + 1, Tp...>(M, N, tol, AA, CC,
                                                           As, Cs);
}

template <typename T>
bool test_send_scalar_adds_view(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  ...
  return test_send_scalar_adds_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA,
                                                                CC, As, Cs);
}

template <typename T, typename MAT_A, typename MAT_C>
bool test_scalar_adds_core(const size_t M, const size_t N, double tol,
                           monolish::matrix::Dense<T> &AA,
...
bool test_scalar_adds(const size_t M, const size_t N, double tol) {
  ...
  return test_scalar_adds_core(M, N, tol, AA, CC, A, C);
}

template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
inline typename std::enable_if<J == sizeof...(Tp), bool>::type
test_scalar_adds_view_core2(const size_t M, const size_t N, double tol,
                            monolish::matrix::Dense<T> &AA,
                            monolish::matrix::Dense<T> &CC, MAT_A &A,
                            std::tuple<Tp...> &Cs) {
  return true;
...
template <typename T, typename MAT_A, std::size_t J = 0, typename... Tp>
    ...
    test_scalar_adds_view_core2(const size_t M, const size_t N, double tol,
                                ...
                                std::tuple<Tp...> &Cs) {
  A = AA;
  if (!test_scalar_adds_core(M, N, tol, AA, CC, A, std::get<J>(Cs))) {
    return false;
  }
  return test_scalar_adds_view_core2<T, MAT_A, J + 1, Tp...>(M, N, tol, AA, CC,
                                                             A, Cs);
}

template <typename T, std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == sizeof...(Tp), bool>::type
test_scalar_adds_view_core1(const size_t M, const size_t N, double tol,
                            monolish::matrix::Dense<T> &AA,
                            monolish::matrix::Dense<T> &CC,
                            std::tuple<Tp...> &A, std::tuple<Tp...> &C) {
  return true;
...
template <typename T, std::size_t I = 0, typename... Tp>
    ...
    test_scalar_adds_view_core1(const size_t M, const size_t N, double tol,
                                ...
                                monolish::matrix::Dense<T> &CC,
                                std::tuple<Tp...> &As, std::tuple<Tp...> &Cs) {
  if (!test_scalar_adds_view_core2(M, N, tol, AA, CC, std::get<I>(As), Cs)) {
    return false;
  }
  return test_scalar_adds_view_core1<T, I + 1, Tp...>(M, N, tol, AA, CC, As,
                                                      Cs);
}

template <typename T>
bool test_scalar_adds_view(const size_t M, const size_t N, double tol) {
  monolish::matrix::COO<T> seedA = get_random_structure_matrix<T>(M, N);
  ...
  return test_scalar_adds_view_core1<T, 0, T1, T2, T3, T4>(M, N, tol, AA, CC,
                                                           As, Cs);
}
