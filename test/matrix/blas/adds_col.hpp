...
bool test_send_adds_col_core(const size_t M, const size_t N, VEC &x,
                             double tol) {
  size_t nnzrow = 27;
  if (nnzrow < N) {
    nnzrow = 27;
...
template <typename MAT, typename T>
bool test_send_adds_col_view_core(const size_t M, const size_t N, double tol) {
  return true;
}
...
bool test_send_adds_col_view_core(const size_t M, const size_t N, double tol,
  ...
  if (!test_send_adds_col_core<MAT, monolish::view1D<U, T>, T>(M, N, vec,
                                                               tol)) {
    return false;
  }
  return test_send_adds_col_view_core<MAT, T, ARGS...>(M, N, tol, args...);
}
...
bool test_send_adds_col_view(const size_t M, const size_t N, double tol) {
  ...
  return test_send_adds_col_view_core<MAT, T, T1, T2, T3>(M, N, tol, x1, x2,
                                                          x3);
}

template <typename MAT, typename T>
bool test_adds_col_view_core(const size_t M, const size_t N, double tol) {
  return true;
}

template <typename MAT, typename T, typename U, typename... ARGS>
bool test_adds_col_view_core(const size_t M, const size_t N, double tol, U x,
                             ARGS... args) {
  monolish::view1D<U, T> vec(x, M / 2, M);
  if (!test_adds_col_core<MAT, monolish::view1D<U, T>, T>(M, N, vec, tol)) {
    return false;
  }
  return test_adds_col_view_core<MAT, T, ARGS...>(M, N, tol, args...);
}
...
bool test_adds_col_view(const size_t M, const size_t N, double tol) {
  ...
  return test_adds_col_view_core<MAT, T, T1, T2, T3>(M, N, tol, x1, x2, x3);
}
