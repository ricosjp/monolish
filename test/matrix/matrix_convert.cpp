...
template <typename MAT, typename T>
bool test_convert2(const size_t M, const size_t N) {
  // ans COO (source)

  std::vector<int> row = {0, 0, 9, 9}, col = {0, 9, 0, 9};
  std::vector<T> value = {1.0, 2.0, 3.0, 4.0};
  monolish::matrix::COO<T> ans_coo(10, 10, 4, row.data(), col.data(), value.data());
...
int main(int argc, char **argv) {
  ...
  if (test_convert2<monolish::matrix::Dense<double>, double>(M, N) == false) {
    return 1;
  }
  if (test_convert2<monolish::matrix::CRS<double>, double>(M, N) == false) {
    return 1;
  }

  if (test_convert2<monolish::matrix::Dense<float>, float>(M, N) == false) {
    return 1;
  }
  if (test_convert2<monolish::matrix::CRS<float>, float>(M, N) == false) {
    return 1;
  }
