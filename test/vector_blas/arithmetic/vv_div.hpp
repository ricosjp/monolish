...
template <typename T>
void ans_vvdiv(monolish::vector<T> &mx, monolish::vector<T> &my, monolish::vector<T> &ans) {
  if (mx.size() != my.size()) {
...
template <typename T>
bool test_send_vvdiv(const size_t size, double tol){
...
template <typename T>
bool test_vvdiv(const size_t size, double tol){
