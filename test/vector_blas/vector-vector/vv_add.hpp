...
template <typename T>
void ans_vvadd(monolish::vector<T> &mx, monolish::vector<T> &my, monolish::vector<T> &ans) {
  if (mx.size() != my.size()) {
...
template <typename T>
bool test_send_vvadd(const size_t size, double tol){
...
template <typename T>
bool test_vvadd(const size_t size, double tol){
