...
template <typename T>
void ans_vvmul(monolish::vector<T> &mx, monolish::vector<T> &my, monolish::vector<T> &ans) {
  if (mx.size() != my.size()) {
...
template <typename T>
bool test_send_vvmul(const size_t size, double tol){
...
template <typename T>
bool test_vvmul(const size_t size, double tol){
