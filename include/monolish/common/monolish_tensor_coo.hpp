...
public:
  /**
   * @brief Coodinate format index, which stores index numbers of the non-zero elements (size nnz)
   */
  ...
  tensor_COO()
    : shape(), gpu_status(false), index(), vad_nnz(0) {
  ...
  [[nodiscard]] bool equal(const tensor_COO<Float> &tens, bool compare_cpu_and_device = false) const;
